import React, { useState, useContext, useRef } from 'react';
import axios from 'axios';
import { Redirect, Link } from 'react-router-dom';
import styled from '@emotion/styled';
// import a component that gives the option to register or login
// then nest the log in and register components within the parent component
import ApiContext from './ApiContext';
import GroceryImage  from './images/grocery-app-image.png';
import GoogleRegister from './RegisterHooks.jsx';

const WrapperDiv = styled.div`
    display: flex;
    justify-content: center;
    align-items: center;
    margin-bottom: 20px;
    input[type='text'], input[type='password'] {
      width: 250px;
      border-radius: 17px;
      height: 30px;
      border: none;
      padding: 10px;
      font-size: 16px;
      color: black;
    }

    img {
      height: 25px;
      width: 25px;
    }
`;

const WrapperDivParent = styled.div`
    background-image: linear-gradient(to bottom, coral , grey);
    padding-bottom: 40px;
    display: flex;
    align-items: center;
    height: 100vh;
    width: 100%;
    justify-content: center;
    .google-register {
      max-width: 300px;
      width: 100%;
      position: absolute;
      top: 75%;
    }
    .login-wrapper {
      position: absolute;
      bottom: 0;
      width: 100%;
      max-width: 420px;
      width: 100%;
    }
`;

const FormContentWrapper = styled.div`
    max-width: 80%;
    margin: 0 auto;
    position: absolute;
    top: 50%;
    transform: translateY(-50%);
`;

const ButtonDiv =  styled.div`
    display: flex;
    justify-content: center;
    align-items: center;
    margin-bottom: 20px;  
    button {
      width: 7em;
      height: 4em;
      background-color: rgb(243, 242, 242);
      border: none;
      border-radius: 30px;
      transition: 0.5s ease-out;
      font-family: 'Big Shoulders Text', cursive;
      font-size: 16px;
      font-weight: bold;
      color: rgb(117, 117, 117);
    }
  `;  

  const ImageDiv = styled.div`
    display: flex;
    justify-content: center;
      img {
        text-align: center;
        height: 60px;
        width: 60px
      }
  }`;

const TogglePasswordVisibility = styled.button`
  border-radius: 4px;
  background-color: grey;
  color: black;
  position: absolute;
  right: 8%;
  width: 50px;
  height: 35px;
  border: none;
}`;

const ForgotDiv = styled.div`
      font-size: 14px;
    text-align: center;
    a {
      color: rgb(243, 242, 242);
      transition: 0.2s ease-in-out;
    }
  }`;

const LoginHeader = styled.div`
  h1 {
    margin: 0;
    text-align: center;
    color: white;
    padding: 1em 0;
  }
  }`;

const LoginButton =  styled.div`
    display:flex;
    align-items: center;
    position: absolute;
    bottom: 0;
    right: 0;
    margin-left: auto;
    justify-content: center;
    height: 3em;
      width: 4em;
    background-color: rgb(243, 242, 242);
    .login-link {
      border: none;
      transition: 0.5s ease-out;
      text-decoration: none;
      font-family: 'Big Shoulders Text', cursive;
      font-size: 16px;
      font-weight: bold;
      color: rgb(117, 117, 117);
    }
  `;  

const LoginDiv = styled.div`
    display: flex;
    justify-content: center;
    align-items: center;
    margin-top: 25px;
    height: 80px;
  `;

export default function(props) {
  // const [userId, setUserId] = useState('');
  const passwordInputEl = useRef(null);
  const passwordToggleButton = useRef(null);
  const formStyle = {
    maxWidth: '350px',
    maxHeight: '525px',
    background: 'linear-gradient(#e66465, #9198e5)',
    borderRadius: '2%'
  }
  // const redirect = props.register;
  const { redirect, setRedirect, userId, setUser, setLoadingModal } = props;
  // const [password, setPassword] = useState('');
  // const handleChangePass = (e) => {
  //   console.log('setting the userId ', e.target.value);
  //   setPassword(e.target.value);
  // } 
  // const handleChangeUsername = (e) => {
  //   console.log('setting the userId ', e.target.value);
  //   setUserId(e.target.value);
  // } 

  const togglePass = (e) => {
   if (passwordInputEl.current.type === 'password') {
     passwordInputEl.current.type = 'text'
     passwordToggleButton.current.innerHTML = 'Hide';
    } else {
      passwordInputEl.current.type = 'password';
      passwordToggleButton.current.innerHTML = 'Show';
    }
    e.preventDefault();
  }

  const api = useContext(ApiContext);
  const sendUserInfoToDatabase = (event) => {
    const username = document.querySelector('input[name=username]').value;
    const password = document.querySelector('input[name=password]').value;

    console.log('submitting a user');
    event.preventDefault();
    console.log('userId ', userId);
    axios.post(api + '/register', {
      userId: username,
      password: password
    })
    .then((response) => {
      // handle success
      setRedirect(true);
      setUser(username);
      console.log(response);
    })
    .catch((error) => {
      // handle error
      console.log(error);
    });
  };
  
  return (
  // <div>
  //   <p>Log In</p>
  //   <form>
  //     <label>
  //     Username:
  //       <input type="text" name="Username" value={userId} onChange={handleChangeUsername} />
  //     </label>
  //     <label>
  //     Password:
  //       <input type="text" name="Password" value={password} onChange={handleChangePass} />
  //     </label>
  //     <input type="submit" value="Submit" onClick={onSubmit} />
  //   </form>
  // </div>
    // Copy and paste this into your project. Height is advised to be left as is. everything else can be manipulated to your likings.

   
    // function used to control submit event on form.
    
      // The prop names must be identical to whats shown below or the component would not render.
      <React.Fragment>
       {redirect ? <Redirect to="/" /> : <WrapperDivParent>
          <FormContentWrapper>
            {/* <ImageDiv><img src={GroceryImage} alt="" /></ImageDiv> */}
            <ImageDiv><img src={GroceryImage} alt="" /></ImageDiv>
            <LoginHeader><h1>SIGN UP</h1></LoginHeader>
            <form onSubmit={sendUserInfoToDatabase} style={{position: 'relative'}}>
              <WrapperDiv><img src="data:image/png;base64," alt="" /><input type="text" placeholder="Username" name="username" /></WrapperDiv>
              <WrapperDiv><img src="data:image/png;base64," alt="" /><input type="password" placeholder="Password" name="password" ref={passwordInputEl} /><TogglePasswordVisibility onClick={togglePass} ref={passwordToggleButton}>SHOW</TogglePasswordVisibility></WrapperDiv>
              <ButtonDiv><button type="submit" >SIGN UP</button></ButtonDiv>
              {/* <ForgotDiv>
                  <p><a href="#">Forgot Password?</a></p>
              </ForgotDiv> */}
            </form>
          </FormContentWrapper>
          <GoogleRegister setLoadingModal={setLoadingModal} setRedirect={setRedirect} setUser={setUser} />
          <div className="login-wrapper"><LoginButton><div><Link className="login-link" to="/login">Login</Link></div></LoginButton></div>
        </WrapperDivParent>
        }
      </React.Fragment>
    );
}